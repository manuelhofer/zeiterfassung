...
CREATE TABLE `auftragszeit` (
  ...
  KEY `idx_auftragszeit_arbeitsschritt` (`arbeitsschritt_id`),
...
CREATE TABLE `terminal` (
  ...
  `aktiv` tinyint(1) NOT NULL DEFAULT 1,
  `db_benutzer` varchar(80) DEFAULT NULL COMMENT 'Datenbankbenutzer dieses Terminals (aus der Kopplung)',
  `db_benutzer_host` varchar(60) DEFAULT NULL COMMENT 'Host-Muster des Datenbankbenutzers, z. B. % oder 192.168.10.%',
  `gekoppelt_am` datetime DEFAULT NULL COMMENT 'Zeitpunkt der letzten erfolgreichen Kopplung',
  `gekoppelt_host` varchar(190) DEFAULT NULL COMMENT 'Kennung des Geraets (Hostname/MAC) aus der letzten Kopplung',
  `erstellt_am` datetime NOT NULL DEFAULT current_timestamp(),
  `geaendert_am` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
