...
class MitarbeiterAdminController
{
     ...
     * `views/mitarbeiter/formular.php` genau das, und deshalb bleibt diese
     * Konstante stehen: Kommt ein zweiter Anwendungsfall dazu, ist hier der
     * Ort, an dem der Stand dokumentiert ist.
     *
     * Die Sperre aus P-2026-08-10-25 war nötig, solange der Abteilungsbezug
     * gar nichts bewirkte.
     */
        ...
        if ($mitarbeiterIdNachSave !== null) {
            ...
            if (count($scopeAbtRowIds) > 0 || count($scopeAbtDelIds) > 0 || ($scopeAbtAddRolleId > 0 && $scopeAbtAddAbteilungId > 0)) {
                ...
                try {
                    ...
                    //
                    // Die Konstante bleibt als Schalter stehen: Sie sagt, ob der
                    // Abteilungsbezug überhaupt irgendetwas bewirkt. Heute tut er
                    // das für die Urlaubsgenehmigung (`UrlaubGenehmigungService`);
                    // wäre das wieder nicht der Fall, dürfte die Maske auch keine
                    // Zeilen mehr anlegen, die nichts bewirken – das war der Grund
                    // für die Sperre in P-2026-08-10-25.
