...
*Gilt fuer:* `services/AuftragszeitService.php`, `services/BarcodeService.php`,
...
Auftraege werden **nicht** in diesem System gefuehrt, sondern zu 100 % in einem
externen CMS/ERP. In der Zeiterfassung wird nur die **Auftragsnummer**
verwendet.

Seit 2026-08-08 koennen Auftraege **zusaetzlich** im Backend angelegt werden
...
**Recht** im Weg stehen – `AUFTRAEGE_VERWALTEN` gilt ausschliesslich fuer die
Pflege im Backend.
...
- Der Code wird kurz sichtbar (ca. 50–100 ms), danach wird automatisch ENTER
  ausgeloest und der Request abgeschickt.
- Der Anwender hat keine Gelegenheit, den Code zu bearbeiten – das Feld dient
  nur der Sichtkontrolle.

Auftrags-RFIDs duerfen **nicht** mit Mitarbeiter-RFIDs kollidieren – notfalls
ueber das Format oder ueber die Erkennung nach Kontext.
...
fuer wiederkehrende Taetigkeiten – einmal pflegen, Codes beliebig oft
ausdrucken.

## 5. Haupt- und Nebenauftraege

Ein Mitarbeiter kann **einen** Hauptauftrag laufen haben und zusaetzlich
**mehrere** Nebenauftraege gleichzeitig.

- **Hauptauftrag starten:** Auftragscode scannen, Maschine waehlen/scannen; ein
  vorhandener Hauptauftrag wird automatisch geschlossen.
- **Nebenauftrag starten:** wie Hauptauftrag, aber `typ = 'neben'`. Serverseitig
  **geblockt, wenn kein Hauptauftrag laeuft** (online wie offline).
- **Auftrag stoppen:** Standard ist der gescannte Auftragscode; Rueckfall ist
...
im Praxis-Test als unerwuenscht erwiesen (P-2026-01-18-35 → -36).

Der Arbeitsschritt-Code ist beim Start eines Hauptauftrags **Pflicht**
(Server-Validierung, nicht nur im Formular).

Im Startbildschirm sind die Auftrags-Knoepfe kontextabhaengig: Laeuft ein
...
Fuer Auftrag und Arbeitsschritte werden **Strichcodes (Code 128)** erzeugt –
derselbe Codetyp wie bei den Maschinen, passend zu den 1D-Handscannern im
Betrieb.

**Der Code enthaelt nur den nackten Wert**, damit das Terminal ihn unveraendert
uebernimmt.
...
- **Kartenblatt** je Katalogschritt (`?seite=arbeitsschritt_katalog_blatt`) mit
  Stueckzahl – z. B. 20 Karten `fraesen` fuer 20 Fraesmaschinen. Sechs Karten je
...
Recht: `AUFTRAEGE_VERWALTEN` nur fuer das Pflegen. **Ansehen, Detailansicht und
